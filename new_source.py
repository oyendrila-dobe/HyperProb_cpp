...
def SemanticsFuture(model, formula_duplicate, n):
    ...
    while i >= 0:
        ...
        add_to_variable_list(prob_phi)
        new_prob_const = "(" + prob_phi + " >= 0)"
        first_implies = "(implies(" + holds1 + ", " + prob_phi + " == 1) && " + new_prob_const + ")"
        nos_of_subformula += 1
        ...
        for ca in combined_acts:
            name = 'a_' + str(r_state[rel_quant[0] - 1])
            add_to_variable_list(name)
            act_str = "(" + name + " == " + str(ca[0]) + ")"
            if len(rel_quant) > 1:
                for l in range(2, len(rel_quant) + 1):
                    name = 'a_' + str(rel_quant[l - 1] - 1)
                    add_to_variable_list(name)
                    act_str = "(" + act_str + " && (" + name + " == " + str(ca[l - 1]) + "))"

            implies_precedent = "(!" + holds1 + " && " + act_str + ")"
            nos_of_subformula += 2
            ...
            for cs in combined_succ:
                ...
                # resume here to create list of strings
                list_of_ors.append("(" + holds_succ + " || (" + d_current + " > " + d_succ + "))")
                nos_of_subformula += 2

            implies_antecedent_and1 = "(" + prob_phi + " == " + str(prod_left) + ")"
            nos_of_subformula += 1
            if len(list_of_ors) != 0:
                prod_right_or = "(" + list_of_ors[0]
                for li in list_of_ors[1:]:
                    prod_right_or += " || " + li
                prod_right_or += ")"
                nos_of_subformula += 1
                implies_antecedent_and2 = "implies(" + prob_phi + " > 0, " + prod_right_or + ")"
                nos_of_subformula += 1
                implies_antecedent = "(" + implies_antecedent_and1 + " && " + implies_antecedent_and2 + ")"
                nos_of_subformula += 1
                file_str = "    b = (" + first_implies + " && (implies(" + implies_precedent + ", " + implies_antecedent + ")));\n    s.add(b);\n"
                nos_of_subformula += 1
                f_pointer.write(file_str)
...
def Semantics(model, formula_duplicate, n):
    global nos_of_subformula, file_str
    r_state = [0 for ind in range(n)]
    rel_quant = []
    if formula_duplicate.data == 'true':
        ...
        while i >= 0:
            ...
            if i >= 0:
                index[i] += 1
                r_state[i] = index[i]
        and_for_yes = list(and_for_yes)
        and_for_no = list(and_for_no)
        flg = True
        if len(and_for_yes) != 0:
            file_str = "    b = ((" + and_for_yes[0]
            for yes in and_for_yes[1:]:
                file_str += " && " + yes
            file_str += ") && (" + and_for_no[0]
        else:
            flg = False
            file_str = "    b = (" +and_for_no[0]
        for no in and_for_no[1:]:
            file_str += " && " + no
        if flg:
            file_str += "));\n    s.add(b);\n"
        else:
        ...
        while i >= 0:
            ...
            file_str1 = "(" + name1 + " && " + name2 + " && " + name3 + ")"
            file_str2 = "(!" + name1 + " && (!" + name2 + " || !" + name3 + "))"
            nos_of_subformula += 2
            file_str = "    b = (" + file_str1 + ") || (" + file_str2 + ");\n    s.add(b);\n"
            nos_of_subformula += 1
            f_pointer.write(file_str)
            while i >= 0 and (index[i] == (len(model.states) - 1) or (i + 1) not in rel_quant):
                r_state[i] = 0
        ...
        while i >= 0:
            ...
            name3 += '_' + str(index_of_phi2)
            add_to_variable_list(name3)
            and_eq = "(" + name1 + " && (" + name2 + " == " + name3 + "))"
            nos_of_subformula += 1
            and_not_eq = "(!" + name1 + " && (" + name2 + " != " + name3 + "))"
            nos_of_subformula += 1
            file_str = "    b = " + and_eq + " || " + and_not_eq + ";\n    s.add(b);\n"
            nos_of_subformula += 1
    ...
    for i in range(n - 1, -1, -1):
        ...
        for j in range(limit):
            ...
            if count == len(model.states) - 1:
                ...
                file_str = ""
                if list_of_AV[i] == 'V':
                    for hold in list_of_holds[index: index + count + 1]:
                        file_str += hold + " || "

                elif list_of_AV[i] == 'A':
                    for hold in list_of_holds[index: index + count + 1]:
                        file_str += hold + " && "
                file_str = "( " + file_str[0:len(file_str)-4] + " )"
                list_of_holds_replace.append(file_str)
                count = -1
                quo += 1
        list_of_holds = copy.deepcopy(list_of_holds_replace)
        list_of_holds_replace.clear()
    file_str = "    b = " + list_of_holds[0] + " ;\n    s.add(b);\n"
    f_pointer.write(file_str)
...
def add_to_variable_list(name):
    ...
    if name[0] == 'h' and not name.startswith('holdsToInt') and name not in list_of_bools:
        ...
        file_str = "    expr " + name + " = c.bool_const(\"" + name + "\");\n"
        f_var_pointer.write(file_var_str)
        f_pointer.write(file_str)
        file_var_str = name + " bool\n"
        f_var_pointer.write(file_var_str)
        ...
        file_str = "    expr " + name + " = c.real_const(\"" + name + "\");\n"
        f_pointer.write(file_str)
        file_var_str = name + " real\n"
        f_var_pointer.write(file_var_str)

    elif name[0] == 'a' and name not in list_of_ints:
        list_of_ints.append(name)
        file_str = "    expr " + name + " = c.int_const(\"" + name + "\");\n"
        f_pointer.write(file_str)
        file_var_str = name + " int\n"
        f_var_pointer.write(file_var_str)

    elif name not in list_of_bools and name not in list_of_reals and name not in list_of_ints:
        raise Exception("Unknown variable type: not bool, int or real")


def check_result(mdp_model, filename):
    ...
    print("Time to link = " + str(time.perf_counter() - starting)) #find less time consuming way to link
    ...
    if process.returncode == 0:
        ...
        print("Time to compile = " + str(time.perf_counter() - starting))
        if process_com.returncode == 0:
            ...
            process_run = subprocess.run(run_cmd, universal_newlines=True, stderr=subprocess.PIPE,
                                         stdout=subprocess.PIPE)
            print("Time to run = " + str(time.perf_counter() - starting))
...
def main_smt_encoding(model, formula_initial, formula, filename):
    global nos_of_subformula, file_str, f_pointer
    list_of_states = []
    starttime = time.perf_counter()
    file_str = "    expr b = c.bool_val(true);\n"
    f_pointer.write(file_str)
    for state in model.states:
        ...
        for action in state.actions:
            if flag:
                file_str = "    b = " + name + " == " + str(
                    action.id)  # try later to see if adding to expr_vector and popping later into an eqn is faster
                flag = False
            else:
                file_str += " || " + name + " == " + str(action.id)
        file_str += ";\n    s.add(b);\n"
        f_pointer.write(file_str)
        nos_of_subformula += 1
    ...
    if formula_initial.data == 'exist_scheduler':
        ...
        print("Checking...")
        f_pointer.write("    std::cout << s.check() << \"\\n\";\n")
        f_pointer.write("}")
        f_pointer.close()
        f_var_pointer.close()
        check_result(model, filename)
        if res:
        ...
        f_var_pointer.close()
        print("Checking...")
        res, li_a, statis, z3time = check_result(model, filename)
        if res:
            print("The property DOES NOT hold!")
        ...
        f_pointer.close()  # all cpp code is written
...
if __name__ == '__main__':
    ...
    file_name = part_path[last_occurrence + 1:]  # with extension
    split_of_file_name = file_name.split('.')
    filename = split_of_file_name[0]
    ...
    # initializing cpp file writer
    f_pointer = open("cpp_files/" + filename + ".cpp", "w")
    f_var_pointer = open("cpp_files/var.txt", "w")
    file_str = "#include<vector>\n#include\"z3++.h\"\n using namespace z3;\n"
    # declaration of main and a context
    file_str += "int main()\n { \n    context c;\n    solver s(c);\n"
    f_pointer.write(file_str)

    main_smt_encoding(initial_model, parsed_formula_initial, formula, filename)
